...
export function renderSigintCard(pkg: SigintPackage): RenderedSigintCard {
  ...
  const topAsset = headlines[0]
    ? { name: headlines[0].asset, country: headlines[0].theater, alignment: headlines[0].alignment }
    : null;
  const excerpt =
    pkg.sources.map((s) => s.excerpt).find((e) => e && e.trim()) || topHeadlines[1] || "";
...
export function sortSigintByBlackSite(packages: SigintPackage[]): SigintPackage[] {
  return [...packages].sort((a, b) => {
    ...
    const gapA = isBlackSiteA
      ? Math.max(...Object.entries(spreadA).map(([_, c]) => (totalA - c) / totalA))
      : 0;
    const gapB = isBlackSiteB
      ? Math.max(...Object.entries(spreadB).map(([_, c]) => (totalB - c) / totalB))
      : 0;
