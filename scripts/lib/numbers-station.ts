...
export function broadcastNumbersStation(): string {
  ...
  const meta = getMeta();
  const selected = selectBroadcastSigint(packages);
  ...
  const radarHtml = packages.map(p => {
  ...
  const sigintListHtml = selected.map(item => {
    ...
    return '<div style="margin-top:15px;padding:10px;border-left:3px solid ' + (isBlackSite ? '#E74C3C' : '#2ECC71') + ';background:#fefefe;">' +
      '<strong style="color:#2c3e50;">' + pkg.topHeadlines[0] + '</strong>' +
      '<span style="font-size:0.9em;color:#7f8c8d;">(' + pkg.id + ')</span>' +
      '<p style="margin:5px 0 10px 0;font-size:0.9em;">' + (pkg.sources.map(src => src.excerpt).find(e => e && e.trim()) || pkg.topHeadlines[1] || '') + '</p>' +
