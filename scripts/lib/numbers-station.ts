...
export function selectBroadcastSigint(
  packages: SigintPackage[],
  max = 5
): { pkg: SigintPackage; isBlackSite: boolean; signalStrength: number }[] {
...
export function broadcastNumbersStation(packages?: SigintPackage[]): string {
  ...
  const radarHtml = sigintPackages
    .map((p) => {
      const score = Math.min(p.assetCount || p.sources.length || 1, 5);
      const hue = (score - 1) * 60;
      return (
        '<span style="display:inline-block;width:14px;height:14px;margin:2px;border-radius:3px;background:hsl(' +
        hue +
        ',70%,50%);"></span>'
      );
    })
  ...
  const sigintListHtml = selected
    .map((item) => {
      const pkg = item.pkg;
      const total = pkg.assetCount || pkg.sources.length || 1;
      const spread = pkg.alignmentSpread || {};
      const isBlackSite = item.isBlackSite;
      const gapPct = isBlackSite
        ? Math.round(Math.max(...Object.entries(spread).map(([_, c]) => (total - c) / total)) * 100)
        : 0;
      const silentSector = isBlackSite
        ? Object.entries(spread).find(([_, c]) => c / total < 0.2)?.[0] || ""
        : "";

      return (
        '<div style="margin-top:15px;padding:10px;border-left:3px solid ' +
        (isBlackSite ? "#E74C3C" : "#2ECC71") +
        ';background:#fefefe;">' +
        '<strong style="color:#2c3e50;">' +
        (pkg.topHeadlines?.[0] || pkg.top_headlines?.[0] || "Untitled signal") +
        "</strong>" +
        '<span style="font-size:0.9em;color:#7f8c8d;">(' +
        pkg.id +
        ")</span>" +
        '<p style="margin:5px 0 10px 0;font-size:0.9em;">' +
        (pkg.sources.map((src) => src.excerpt).find((e) => e && e.trim()) ||
          pkg.topHeadlines?.[1] ||
          pkg.top_headlines?.[1] ||
          "") +
        "</p>" +
        (isBlackSite
          ? '<span style="background:#E74C3C;color:white;padding:2px 6px;border-radius:3px;font-size:0.75em;">BLACK SITE: ' +
            formatSilentSector(silentSector) +
            " (" +
            gapPct +
            "%)</span>"
          : "") +
        '<a href="/botwavebomba/sigint.html?id=' +
        encodeURIComponent(pkg.id) +
        '" style="color:#3498db;text-decoration:none;font-weight:bold;">[FULL INTEL] →</a>' +
        "</div>"
      );
    })
    .join("");
  ...
  const readMin = Math.max(
    1,
    Math.round(sigintPackages.reduce((a, s) => a + (s.assetCount || s.sources.length), 0) * 0.15)
  );

  return (
    '<!DOCTYPE html>\n<html lang="en">\n<head>\n  <meta charset="UTF-8">\n  <title>NISA Numbers Station | ' +
