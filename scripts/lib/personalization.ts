...
export function filterStoriesForYou(stories: Story[], prefs: UserPreferences): Story[] {
  ...
  return stories.filter((story) => {
    ...
    const blocMatch =
      prefs.followedBlocs.length === 0 || prefs.followedBlocs.some((b) => storyBlocs.includes(b));
...
export function getLocalStories(stories: Story[], prefs: UserPreferences): Story[] {
  ...
  return stories
    .filter((story) => story.countries?.some((c) => countries.includes(c)) ?? false)
    .slice(0, 20);
}
